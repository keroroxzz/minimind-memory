...
**唯一允許的措辭**（Codex [186] 逐字收窄，比我提的版本更窄）：

> **在這三個已跑的 seed 中，顯式 true-conjunction 的 `+` 臂拿掉跨事件 state 後，
> end utility 沒有可見下降；`-` 臂與原 `MF0-C` 則有可變的下降。**

這表示**該介入改變了這個訓練配方對 recurrent state 的行為依賴**。

⚠️ **它不表示** state 內部「正在算 conjunction」；
**不表示**容量被「釋放」或「佔用」；
**更不能**推出「容量不是瓶頸」—— 我先前寫的那句已刪。

⚠️ 這個對照**不是**事前登記的檢定，**只可作描述**；
**不得**用它翻轉主判定，也**不得**據此再開新配方
（Codex [185]：無論結果均封存 formation 這條配方；[186]：不開第三個 formation 變體）。

⚠️ **逐 seed 的 `h=0` 原始數值保留於 artifact／results table**，
但**不得**把 `-` 臂那三個點的單調差異寫成研究觀察、待解模式或趨勢：
`n=3`、非 prereg、且**沒有做跨 seed 的 trend test**。
正文最多報其**範圍／異質性** —— **不把視覺排列升格成訊號。**
...
且**該介入改變了此配方對 recurrent state 的行為依賴**（描述性）。
...
### G. 過程錯誤存查（Codex [195] 精確化）

本輪由 Codex 攔下的錯誤，兩類：

**推論類 —— 形狀都是把「沒有觀察到」寫成「不存在」**

- `Δ=68pp` 讀成 headroom → 實為 **hindsight variance**（§4.70／§4.71）
- 「NLL 增益 ≈ 0 ⇒ 排序 ≈ 隨機」→ 實測為**負向**（§4.71）
- 「state 被 conjunction 佔用 ⇒ 容量不是瓶頸」→ GRU 本身即非線性，**推不出來**（§4.73）
- 「`(a)=0` ⇒ 兩側完全不一致」→ 只代表**沒達成**（§4.74）
- 「`(b)=0` 因為沒有輸出是 `K*`」→ 未驗證；正確為 **non-diagnostic**（§4.74）
- 「最後 batch loss ≈ 0 ⇒ 背得完美」→ 那**只是一個 mini-batch**（§4.74）

**流程類**：artifact 缺 negative render；`(c)` 用 code-key 而非 four-atom `RWAKey`；
送信前未驗落檔（兩次）；watcher 的成功訊息觸發失敗條件。

> ⚠️ **精確表述（Codex [195]）**：這些錯誤**曾改變「哪些 subclaim／artifact 狀態可被採納」**
> —— 尤其 **negative carrier** 與 **code-key `(c)`**（前者讓 gate (b)/(c) 一度無輸入可測，
> 後者使 `(c)` 只能算 shadow）。
> **在完成 audit 之後，它們沒有推翻仍存活的 primary gate 判定。**
>
> **不得**寫成「它們從未影響判讀」—— 我先前的措辭正是如此，**已更正**。
...
## 11. 更正（2026-08-13）：§4.1 的「固定深度證明上不可解」是**不成立的推論**

寫理論筆記 `theory/separation.tex` 時，我們自己列了一條 **prohibited shortcut**
（`rem:shortcuts`）：把 `TC⁰` 上界和 `NC¹`-完備並置，**不能**推出無條件的
transformer 下界 —— 那需要 **`TC⁰ ≠ NC¹`，而這是 open problem**。

然後在 repo 內部審查時發現，**我們自己就犯了這條**：

- `research.md` §4.1（本檔第 241 行）：「**S₅ 字問題是 NC¹-完備（固定深度證明上不可解）**。
  這不是經驗法則，是可以事先推導的。」
- `CLAUDE.md`：「NC¹-complete, so a fixed-depth transformer (≈TC⁰) ***provably* cannot** solve
  arbitrary lengths」

**兩處的『證明上／provably』都是錯的**，且第一處還額外宣稱「不是經驗法則」——
正好把唯一站得住的那個依據（實測）給否認掉了。

**可以講的版本**：在 log-precision／saturated 這兩種形式化下，固定深度 transformer 落在
`TC⁰`；S₅ 字問題是 `NC¹`-hard，因此**不知道**它在 `TC⁰` 內；而 §4.1 之後那條
**k≈3 天花板是實測**。任務選擇不受影響 —— 被推翻的只有「這是事先可證明的」這個宣稱。

依「不回填」原則，§4.1 原文保留不改；`CLAUDE.md`（活文件、會被當指引讀）已就地更正。
